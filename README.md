...
Encore à faire : lancer une Incantation achetée, le quorum de passage de phase ("tout le monde prêt" plus tôt que le minuteur fixe) et surtout le branchement sur `ArenaBattle.gd` — la scène reste aujourd'hui pilotée uniquement en solo local ; c'est la seule pièce qui reste, et la plus risquée (aucune UI réseau Arena n'existe encore, et le fichier est aujourd'hui entièrement pensé pour un seul joueur humain local).
