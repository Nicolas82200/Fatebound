...
func apply_rarity_style() -> void:
	...
	style.border_color = RARITY_COLORS.get(data.rarity, Color.WHITE)
	rarity_panel.add_theme_stylebox_override("panel", style)
	


func apply_race_style() -> void:
	var style := StyleBoxFlat.new()
	style.bg_color = RACE_COLORS.get(data.race, Color.WHITE)
	text_background.add_theme_stylebox_override("panel", style)
	
	var border_style := StyleBoxFlat.new()
	
	border_style.bg_color = Color.TRANSPARENT

	border_style.border_width_left = 8
	border_style.border_width_right = 8
	border_style.border_width_top = 15
	border_style.border_width_bottom = 0
	border_style.border_color = BORDER_RACE_COLORS.get(
		data.race,
		Color.WHITE
	)
	border_style.border_blend = true
	border_color.add_theme_stylebox_override(
		"panel",
		border_style
	)
