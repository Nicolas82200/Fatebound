...
**Protocole de partie Arena (`ArenaGameCommand`)**, en modèle **hôte autoritaire** : un client envoie une `REQUEST_*` (achat/vente/reroll/XP/gel/pose/déplacement) à l'hôte pour agir sur SON PROPRE siège ; l'hôte, seul à exécuter réellement `ArenaMatch` (`ArenaHostAuthority.apply`, pur, sans validation d'autorisation — celle-ci revient à l'appelant réseau qui connaît la correspondance `peer_id -> seat_id` établie par le handshake), diffuse ensuite un `BOARD_SYNC` (état PUBLIC complet du siège — plateau + PV du héros, jamais la main/boutique) que chaque client applique via `ArenaRemoteBoardMirror.apply`. Choix délibéré : resynchroniser tout l'état public plutôt qu'un delta d'action, pour éliminer tout risque de dérive entre clients (le coût en bande passante est négligeable, un plateau Arena reste petit). `ArenaBoardSnapshot` (dé)sérialise un plateau (`Array[Minion]` <-> `Array[Dictionary]` de types de base, seul format transmissible par `var_to_bytes`) ; la résolution sécurisée d'une carte par `resource_path` (chemin restreint au dossier des cartes, jetons refusés) est factorisée dans `NetCardResolver`, réutilisé par `ArenaBoardSnapshot` ET par `NetworkOpponent` (1v1, refactor pur — comportement inchangé). `ArenaMatch.find_by_seat(seat_id)` retrouve le bon `ArenaPlayerState` réseau (jamais `get_instance_id()`). Portée volontairement réduite : Incantations (lancer/vendre un sort) et quorum de passage de phase ("tout le monde prêt") pas encore couverts. **Pas encore branché sur `ArenaBattle.gd`** (la scène reste pilotée en solo local uniquement) : ce protocole est validé en pure logique (`test_arena_board_snapshot.gd`, `test_arena_host_authority.gd`, `test_arena_remote_board_mirror.gd`, `test_net_card_resolver.gd`), sans dépendance à Steam.
