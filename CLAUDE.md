...
**Fin de manche réseau (`ROUND_ADVANCED`/`GAME_OVER`, `ArenaHostRoundSync`)** : le combat lui-même n'était couvert par aucun message réseau — ajouté après coup. Décision (validée avec l'utilisateur) : combat résolu **headless côté hôte** (comme les combats bot-contre-bot en solo), jamais rejoué en direct chez les clients — synchroniser une animation identique entre plusieurs vrais joueurs aurait demandé un flux RNG/évènements parfaitement ordonné entre eux, risque disproportionné pour un protocole jamais testé en conditions réseau réelles. Après `ArenaMatch.start_combat_phase()`, `ArenaHostRoundSync.broadcast_after_combat` diffuse un `BOARD_SYNC` pour **chaque** siège (le combat change potentiellement le plateau de tout le monde, pas seulement d'un émetteur — `ArenaHostGameRouter` ne suffit pas ici) puis `ROUND_ADVANCED` (nouveau `round_number` + résumé texte, même format que `ArenaMatch.last_combat_summaries`) ou `GAME_OVER` (classement final) selon l'état de la partie. `ArenaClientGameLink` expose les signaux `round_advanced`/`game_over` correspondants.
